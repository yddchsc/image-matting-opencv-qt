...
class SLICProcessor(object):
    ...
    def iterate_10times(self):
        ...
        for i in trange(10):
            self.assignment()
            self.update_cluster()
            self.name = 'lenna_M{m}_K{k}_loop{loop}.png'.format(loop=i, m=self.M, k=self.K)
            self.save_current_image(self.name)
    ...
    def start_matting(self):
        ...
        for i in trange(1):
            self.assignment()
            self.update_cluster()
        self.name = 'lenna_M{m}_K{k}_loop{loop}.png'.format(loop=10, m=self.M, k=self.K)
